...
class App(CTk.CTk):
    def __init__(self):
        ...
        self.mc_directory = ".minecraft/versions/"
        ...
        self.swither_mode = config.get('oth_versions_swither', 'mode')
        ...
        def past_version(self):
            ...
            print(print("Выбранная версия: "),self.past_versionid)
        ...
        def start_game():
            ...
            version_directory = self.mc_directory + self.past_versionid
            if 'fabric' in self.past_versionid:
                fabric_version = self.past_versionid.replace(' fabric','')
                print(fabric_version)
                minecraft_launcher_lib.fabric.install_fabric(fabric_version, version_directory, '0.16.0')
                installed_versions = []
                for mc_versions in minecraft_launcher_lib.utils.get_installed_versions(version_directory):
                ...
                if loader_version in installed_versions:
                    subprocess.call(minecraft_launcher_lib.command.get_minecraft_command(version=loader_version, minecraft_directory=version_directory, options=opions))
            else:
                minecraft_launcher_lib.install.install_minecraft_version(versionid=self.past_versionid, minecraft_directory=version_directory)
                subprocess.call(minecraft_launcher_lib.command.get_minecraft_command(version=self.past_versionid, minecraft_directory=version_directory, options=opions))
        ...
        # занесение версий в список versionId
        if self.swither_mode == '1':
        ...
        def settings_win():
            if self.sett_opened == False:
                ...
                self.ram_combobox.grid(row=1, column=0, padx=(10, 10), pady=(40, 10), sticky="w")
                ...
                # тексты
                # RAM
                self.Ram_lable = CTk.CTkLabel(settings_wind, text="Оперативная память", font=("JetBrains Mono Medium", 18))
                self.Ram_lable.grid(row=1, column=0, padx=(10, 10), pady=(5, 45), sticky="w")
                # Директория
                ...
                def versions_swither():
                    self.swither_mode = self.other_versions_swither.get()
                    config['oth_versions_swither'] = {'mode': self.swither_mode}

                self.other_versions_swither = CTk.CTkSwitch(settings_wind, command= lambda:versions_swither(), text='Поддержка снапшотов и бета версий?', font=("JetBrains Mono Medium", 14))
                self.other_versions_swither.grid(row=3, column=0, padx=(10, 10), pady=(10, 10), sticky="w")
                if self.swither_mode == '1':
                    self.other_versions_swither.select(int(self.swither_mode))
                else:
                    pass

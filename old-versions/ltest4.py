...
class App(CTk.CTk):
    def __init__(self):
        ...
        #лого посреди окна
        self.logo = CTk.CTkImage(dark_image=Image.open("../images/LauncherTitle.png"), size=(630, 67))
        self.logo_lavel = CTk.CTkLabel(master=self, text="", image=self.logo)
        self.logo_lavel.grid(row=0, column=0, padx=(10, 0), pady=(30, 0))

        #создание фрейма
        self.ui_frame = CTk.CTkFrame(self, corner_radius=15, width=380, height=300)
        self.ui_frame.grid(row=1, column=0, padx=(224, 0), pady=(105, 30), sticky="nsw")

        #создание фрейма доп кнопок
        self.Buttons_frame = CTk.CTkFrame(self, corner_radius=15, width=20, height=100)
        self.Buttons_frame.grid(row=1, column=0, padx=(20, 0), pady=(225, 30), sticky="nsw")

        #поле ввода ника
        self.nickname_entry = CTk.CTkEntry(master=self.ui_frame, corner_radius=13, width=300, height=50, font=("JetBrains Mono Medium", 20), placeholder_text=self.player_nickname)
        ...
        #меню выбора версий
        self.version_menu = CTk.CTkComboBox(self.ui_frame, corner_radius=13, font=("JetBrains Mono Medium", 20), width=300, height=50, values=versionsId)
        self.version_menu.grid(row=2, column=0, padx=(20, 20), pady=(10, 20))

        #кнопка играть
        self.play_button = CTk.CTkButton(master=self.ui_frame, width=300, height=50, corner_radius=13, border_width=0, text="Играть",font=("JetBrains Mono Medium", 25), command=lambda: start_game_thread())
        self.play_button.grid(row=3, column=0, padx=(20, 20), pady=(10, 20), sticky="w")

        self.toplevel_window = None

        #кнопка настроек
        self.settings_button = CTk.CTkButton(master=self.Buttons_frame, width=30, height=45, corner_radius=13, border_width=0, image=add_settings_image, text="", command=lambda: settings_win())
        self.settings_button.grid(row=3, column=0, padx=(10, 10), pady=(10, 70), sticky="w")

        #кнопка директории
        self.directory_button = CTk.CTkButton(master=self.Buttons_frame, width=30, height=45, corner_radius=13, border_width=0, image=add_info_image, text="", command=lambda: print())
        ...
        def settings_win():
            if self.sett_opened == False:
                ...
                self.ram_combobox = CTk.CTkComboBox(settings_wind, width=110, height=35, values=["1024", "2048", "3072", "4096", "5120", "6144", "7168", "8192"], command=ram_choice)
                ...
                self.select_directory_button = CTk.CTkButton(master=settings_wind, width=30, height=35, corner_radius=13, border_width=0,font=("JetBrains Mono Medium", 15), text="Select Folder", command=lambda: Mc_dir_select())
